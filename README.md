...
| BROWSER_DOWNLOAD_DIR           |            | ${HOME}/Downloads/test_temp_files  | Абсолютная ссылка куда браузер будет скачивать файлы. !!!ОСТОРОЖНО!!! ДАННЫЕ В ДАННОЙ ПАПКЕ УДАЛЯЮТСЯ                                                                                                                                                                           |
...
| GH_OWNER                       | +          |                                    | Имя профиля репозитория в github. Если делали fork репозитория, то нужно указать имя аккаунта.                                                                                                                                                                                  |
| GH_REPO                        | +          |                                    | Имя репозитория в github. Если делали fork репозитория то нужно указать имя аккаунта.                                                                                                                                                                                           |
...
1) Создай внешний docker volume `test_files_volume` командой: `docker volume create test_files_volume` 
2) Открой консоль в папке проекта
3) В папке `env` сделать дубликат `env.local.example` под новым именем `env.docker`
4) Укажи все обязательные значения конфигурации:
   ...
   ...
   и другие
5) При необходимости можешь указать и опциональные переменные значения конфигурации.
`PYTEST_ARGS="-vv"`, `BROWSER_NAME=chrome`, `BROWSER_VERSION=128.0` и т. д.
6) Запускай команду `bash run_project.e2e.sh`
7) Дождись пока все контейнеры поднимутся, а после контейнер `automation-exercise-test` остановиться
8) Открой отчеты allure: `allure serve allure-results`
